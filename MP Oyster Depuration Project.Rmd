...
predictions <- 
  data.frame(sampleday = seq(from = 0,
                              to = 10,
                              length.out = nrow(totalsums)),
             length = rep(mean(totalsums$length, 
                               nrow(totalsums))),
             Table = rep(NA,
                         nrow(totalsums)),
             Tank = rep(NA,
...
predictions$mean <-
  exp(new.predict$fit)
predictions$upper <- exp(with(new.predict, fit + 1.96*se.fit))
predictions$lower <- exp(with(new.predict, fit - 1.96*se.fit))
