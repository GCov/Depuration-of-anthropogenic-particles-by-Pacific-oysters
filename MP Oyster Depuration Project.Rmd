...
depblanks4 <- ddply(depblanks4, names(depblanks4)[-8], summarize,
                   value = sum(value))
```
...
blankssizesums <- ddply(depblanks4, c('id', 'size.cat', 'DOFSample', 
...
blanksprop <- ddply(blankssizesums, 'id', transform, prop = sum/sum(sum))
...
blanksprop3 <- ddply(blanksprop2, c("DOFSample", "size.cat"), summarize,
                     mean = mean(prop))
...
ggplot(blanksprop3, aes(x=DOFSample, y=100*mean, fill = size.cat)) + 
  geom_col(size = 1) + 
  xlab('Sampling Day') + 
  ylab('Average percent') +
  guides(fill=guide_legend(title="Particle size category \n (microns)")) +
...
blankstypesums <- ddply(depblanks4, c('id', 'DOFSample', 'dat.filter', 
                                      'dat.count', 'observer', 'variable'), 
...
blankstypeprop <- ddply(blankstypesums, 'id', transform, prop = sum/sum(sum))
...
blankstypeprop3 <- ddply(blankstypeprop2, c("DOFSample", "variable"), summarize,
                     mean = mean(prop))

## Now plot

ggplot(blankstypeprop3, aes(x=DOFSample, y=100*mean, fill = variable)) + 
  geom_col(size = 0.5, colour = 'black') + 
  xlab('Sampling Day') + 
  ylab('Average percent') +
  guides(fill=guide_legend(title="Particle size category \n (microns)")) +
...
depblanksmean <- ddply(depblanks4, c('size.cat', 'dat.filter', 'variable'),
...
oysterdep4$dat.filter <- as.factor(oysterdep4$dat.filter)
depblanks4$dat.filter <- as.factor(depblanks4$dat.filter)
...
sum(oystersizesums2$sum[oystersizesums2$size.cat == '10-100'])/
  sum(oystersizesums2$sum)*100  # 3.5 %

sum(oystersizesums2$sum[oystersizesums2$size.cat == '100-500'])/
  sum(oystersizesums2$sum)*100  # 40.2 %

sum(oystersizesums2$sum[oystersizesums2$size.cat == '500-1000'])/
  sum(oystersizesums2$sum)*100  # 28.0 %

sum(oystersizesums2$sum[oystersizesums2$size.cat == '1000-5000'])/
  sum(oystersizesums2$sum)*100  # 28.3 %


## Sums by particle type

summary(oystertypesums2$variable)

sum(oystertypesums2$sum[oystertypesums2$variable == 'Blue fragments' | 
                  oystertypesums2$variable == 'Clear fragments' |
                  oystertypesums2$variable == 'Green fragments' | 
                  oystertypesums2$variable == 'Orange fragments' |
                  oystertypesums2$variable == 'Pink fragments' |
                  oystertypesums2$variable == 'Red fragments' |
                  oystertypesums2$variable == 'Turquoise fragments'])/
  sum(oystertypesums2$sum) * 100

sum(oystertypesums2$sum[oystertypesums2$variable == 'Clear spherules' | 
                  oystertypesums2$variable == 'White spherules'])/
  sum(oystertypesums2$sum) * 100
...
depwater4$day <- factor(depwater4$day, 
                        levels = 
                          c('0', '1', '3', '5', '10', 'Control'))  # put in order

watersizesums <- ddply(depwater4, c('id', 'day', 'size.cat', 'Table', 'Tank', 
...
watertypesums <- ddply(depwater4, c('id', 'day', 'Table', 'Tank', 
                                    'DOFSample', 'dat.filter', 'dat.count', 
                                    'observer', 'variable'), 
                        summarize, sum = sum(value))
